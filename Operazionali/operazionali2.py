...
def main():
	...
	for i in range(len(series)):
		...
		yerr=np.sqrt(yerr**2+np.dot(errx,d_model(valx,parametri))**2)  #Calcola l'errore indotto usando l'errore sulle
																		# X, quello sulle Y e la derivata della curva di best fit
	...
	valx,errx=np.array(list(Int.loc[:,'f'])),np.ones(len(yerr))  #Assegna X e Xerr
	parametri=fit(residual,valx,valy)   #Fa il fit, e restituisce i parametri del best fit
...
def model(pars,x):
	...
	#c=vals['c']
	#d=vals['d']
	model = a*np.log(x)  #MODIFICARE PER MODIFICARE LA FUNZIONE DI BEST FIT. RICORDARE DI MODIFICARE IL NUMERO DI PARAMETRI commentando o decommentando 
...
def model1(pars,x):
	...
	c=vals['c']
	#d=vals['d']
	model = a*np.ones(len(x))-b*np.log(x-c*np.ones(len(x)))  #MODIFICARE PER MODIFICARE LA FUNZIONE DI BEST FIT. RICORDARE DI MODIFICARE IL NUMERO DI PARAMETRI commentando o decommentando 
					#quelli non utilizzati: il numero di parametri indipendenti deve essere pari al numero di parametri che python usa, altrimenti
					#il fit non funziona correttamente
